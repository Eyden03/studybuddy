...
$query = trim($_GET['q'] ?? '');
$sessions = fetch_sessions($filter, $query, $user);

function filter_url(string $filter, string $query): string
{
...
<div class="flex">
  ...
  <main class="flex-1 px-8 py-8 max-w-[1200px]">
    ...
    <div class="flex flex-wrap items-center gap-2 mb-7">
      <a href="<?= e(filter_url('all', $query)) ?>" class="<?= filter_class('all', $filter) ?>" style="<?= $filter === 'all' ? 'background:var(--ink)' : '' ?>">All</a>
      <a href="<?= e(filter_url('open', $query)) ?>" class="<?= filter_class('open', $filter) ?>" style="<?= $filter === 'open' ? 'background:var(--ink)' : '' ?>">Open now</a>
      <a href="<?= e(filter_url('free', $query)) ?>" class="<?= filter_class('free', $filter) ?>" style="<?= $filter === 'free' ? 'background:var(--ink)' : '' ?>">Free</a>
      <a href="<?= e(filter_url('near', $query)) ?>" class="<?= filter_class('near', $filter) ?>" style="<?= $filter === 'near' ? 'background:var(--ink)' : '' ?>">Near me</a>
      <a href="<?= e(filter_url('week', $query)) ?>" class="<?= filter_class('week', $filter) ?>" style="<?= $filter === 'week' ? 'background:var(--ink)' : '' ?>">This week</a>
